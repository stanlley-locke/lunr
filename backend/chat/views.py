...
def backup_data(request):
    ...
    Includes: Profile, Contacts, Chat Rooms, Messages, Settings.
    """
    user = request.user
    ...
    for room in rooms:
        ...
        room_datum = room_serializer.data
        
        # Get messages for this room that are visible to the user (not deleted)
        messages = Message.objects.filter(room=room, deleted_at__isnull=True)
        messages_data = MessageSerializer(messages, many=True).data
        
        room_datum['messages'] = messages_data
        rooms_data.append(room_datum)
